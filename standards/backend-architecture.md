...
| OpenTelemetry SDK registration (tracing + metrics) | `Infrastructure/` (per service, folded into `AddGatewayInfrastructure()`/`AddAccountServiceInfrastructure()`), per [../architecture/observability.md](../architecture/observability.md) — matches every other DI/infra registration in this table, not `Program.cs` |
