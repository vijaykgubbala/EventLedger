...
**Self-caught correction (found while starting Phase 5, fixed retroactively in Phase 2's files)**: neither `OnModelCreating` originally called `HasColumnName(...)`, so EF Core defaulted every column to its PascalCase C# property name (`EventId`, `AccountId`, ...), silently violating [standards/naming.md](../../standards/naming.md)'s documented `snake_case` column convention (`event_id`, `account_id`, ...) — confirmed via a schema dump of the actual `CREATE TABLE` SQL. Fixed by adding explicit `HasColumnName(...)` for every property in both `GatewayDbContext` and `AccountDbContext`, with a `Schema_UsesSnakeCaseColumnNames` regression test added to both `*DbContextTests.cs` files. While fixing this, also found and fixed a related latent bug: the `Type` value converter used an unsafe ternary (`type == Credit ? "CREDIT" : "DEBIT"`) that would silently map any out-of-range enum value to `"DEBIT"` rather than rejecting it — replaced with a `switch` that throws `ArgumentOutOfRangeException` (extracted as `TransactionTypeExtensions.ToWireString()`/`ParseWireString()` in each service's `Domain/`, reused by `SubmitEventHandler`'s outbound call too). Also added the `CHECK (type IN ('CREDIT', 'DEBIT'))` constraint documented in [architecture/data-model.md](../../architecture/data-model.md) but never actually implemented — it's now unreachable via the typed C# API (the safe conversion throws first), but is still correct to have as documented schema-level defense-in-depth against any future non-EF-Core write path. Covered by `Insert_UnmappedTransactionTypeValue_ThrowsBeforeReachingDatabase` in both test files.
